...
module completions {
  ...
  export extern "git switch" [
    ...
    --force-create(-C): string # Force create (overwrite)
    --detach(-d)               # Switch to a detached HEAD commit
    --guess                    # Fall back on remote tracking branch (default)
    --no-guess                 # Don't consider remote tracking branches
    --force(-f)                # Alias for --discard-changes
    --discard-changes          # Throw away local changes
    --merge(-m)                # Merge local changes into target branch
    --conflict: string         # Like -m, but specify a conflict style
    --quiet(-q)                # Quiet, suppress feedback messages
    --progress                 # Report progress status (default on terminal)
    --no-progress              # Suppress progress status
    --track(-t)                # Set an upstream branch when creating
    --no-track                 # Do not set an upstream branch
    --orphan: string           # Create a new orphan branch with no tracked files
    --ignore-other-worktrees   # Allow ref to be held by more than one worktree
    --recurse-submodules       # Update submodules to commit recorded in superproject
